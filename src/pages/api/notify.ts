...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  const notifyApiSecret = env.NOTIFY_API_SECRET;
  if (!notifyApiSecret) {
  ...
  try {
    const result = await fetch(`https://notify.walletconnect.com/${projectId}/notify`, {
      method: "POST",
      headers: {
    ...
    return res.status(result.status).json({ success: isSuccessfulGm, message: gmRes?.reason });
  } catch (error: any) {
